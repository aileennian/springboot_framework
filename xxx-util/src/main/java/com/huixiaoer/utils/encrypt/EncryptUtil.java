package com.huixiaoer.utils.encrypt;
...
public class EncryptUtil {
	...
	public static String encrypt(String xmlStr) {
		...
		// 使用Base64加密后返回
		String s=new BASE64Encoder().encode(temp);
		//s=s.replace("/", "xxg");
		return UrlUtil.getURLEncoderString(s);
	}
	...
	public static String decrypt(String xmlStr) throws Exception {
		//xmlStr=xmlStr.replace("xxg", "/");
		xmlStr = UrlUtil.getURLDecoderString(xmlStr);
		// base64解码
	...
	public static void main(String[] args) {
		String jm=encrypt("123456");
		System.out.println(jm);
		try {
			System.out.println(decrypt(jm));
		} catch (Exception e) {
			e.printStackTrace();
		}
